...
function corporate_profile_install_tasks($install_state) {
  $tasks = array();
  $tasks['corporate_profile_custom_settings'] = array(
    'display' => FALSE,
  );
 ...
 * installation state.
 */
function corporate_profile_custom_settings($install_state) {
  $node = node_load(3);
  if (!empty($node)) {
    $node->path = array(
      'alias' => 'about-us',
...
function corporate_profile_public_files_copy($install_state) {
  $source = 'profiles/corporate_profile/node_export_assets/adaptivetheme/';
  $res = 'sites/default/files/adaptivetheme/';
  corporate_profile_recurse_copy($source, $res);
  $image_source = 'profiles/corporate_profile/node_export_assets/logo.png';
  $image_res = 'sites/default/files/logo.png';
  copy($image_source, $image_res);
  $image_source = 'profiles/corporate_profile/node_export_assets/img.jpg';
  $image_res = 'sites/default/files/img.jpg';
  copy($image_source, $image_res);
  $source = drupal_realpath('profiles/corporate_profile/node_export_assets/default_blog_pic.jpg');
  $file = (object) array(
    'uid' => 1,
    'uri' => $source ,
